...
describe('hls builders', () => {
  ...
  before(async () => {
    ...
    buildHlsArgs = m.buildHlsArgs; buildDashArgs = m.buildDashArgs;
  });
  ...
  it('buildHlsArgs includes hls_time', () => {
    const args = buildHlsArgs('in.mp4', '/out', { segmentDuration: 4 });
  ...
  it('buildHlsArgs with hlsFlags', () => {
    const args = buildHlsArgs('in.mp4', '/out', { hlsFlags: 'delete_segments' });
  ...
  it('buildDashArgs with videoCodec and bitrate', () => {
    const args = buildDashArgs('in.mp4', '/out/manifest.mpd', { videoCodec: 'libx264', videoBitrate: '2M' });
    assert.ok(args.includes('libx264'));
    assert.ok(args.includes('2M'));
  });

  it('buildDashArgs with segmentDuration', () => {
    const args = buildDashArgs('in.mp4', '/out/manifest.mpd', { segmentDuration: 4 });
...
describe('subtitle builders', () => {
  ...
  before(async () => {
    const m = await import('../../../dist/esm/helpers/subtitles.js');
    buildBurnSubtitlesFilter = m.buildBurnSubtitlesFilter;
  });
...
describe('waveform builders', () => {
  let buildWaveformFilter: any, buildSpectrumFilter: any;
  before(async () => {
    const m = await import('../../../dist/esm/helpers/waveform.js');
    buildWaveformFilter = m.buildWaveformFilter;
    buildSpectrumFilter = m.buildSpectrumFilter;
  });
...
describe('normalize builders', () => {
  ...
  before(async () => {
    ...
    buildLoudnormFilter = m.buildLoudnormFilter;
  });
...
describe('gif builders', () => {
  let buildGifArgs: any, buildGifPalettegenFilter: any;
  before(async () => {
...
describe('screenshot builders', () => {
  let buildScreenshotArgs: any, buildFrameBufferArgs: any, buildTimestampFilename: any;
  before(async () => {
    const m = await import('../../../dist/esm/helpers/screenshots.js');
    buildScreenshotArgs = m.buildScreenshotArgs;
    buildFrameBufferArgs = m.buildFrameBufferArgs;
    buildTimestampFilename = m.buildTimestampFilename;
  });
...
describe('stream builders', () => {
  let buildPipeThroughArgs: any, buildStreamOutputArgs: any;
  before(async () => {
    const m = await import('../../../dist/esm/helpers/streams.js');
    buildPipeThroughArgs = m.buildPipeThroughArgs;
    buildStreamOutputArgs = m.buildStreamOutputArgs;
  });
...
describe('filter complex coverage', () => {
  let FilterGraph: any, videoFilterChain: any, audioFilterChain: any;
  let filterGraph: any, _GraphNode: any, _GraphStream: any;
  before(async () => {
    const m = await import('../../../dist/esm/filters/complex.js');
    FilterGraph = m.FilterGraph; videoFilterChain = m.videoFilterChain;
    audioFilterChain = m.audioFilterChain; filterGraph = m.filterGraph;
    _GraphNode = m.GraphNode; _GraphStream = m.GraphStream;
  });
...
describe('twopass buildTwoPassArgs', () => {
  let buildTwoPassArgs: any;
  before(async () => {
    const m = await import('../../../dist/esm/helpers/twopass.js');
    buildTwoPassArgs = m.buildTwoPassArgs;
  });
