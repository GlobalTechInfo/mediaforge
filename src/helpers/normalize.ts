 ...
 */
export async function detectSilence(opts: DetectSilenceOptions): Promise<SilenceSegment[]> {
  const {
    input,
    threshold = -50,
 ...
 */
export async function detectScenes(opts: DetectScenesOptions): Promise<SceneChange[]> {
  const {
    input,
    threshold = 0.4,
    sceneList,
 ...
 */
export async function cropDetect(opts: CropDetectOptions): Promise<CropRegion | null> {
  const {
    input,
    limit = 100,
    skip = 5,
