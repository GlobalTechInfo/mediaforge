...
import { twoPassEncode, buildTwoPassArgs } from '../../lib/helpers/twopass.ts';
...
// ─── Module-level setup for integration tests (top-level await) ───────────────
const srcDir = await mkdtemp(join(tmpdir(), 'ffmpeg-ts-twopass-src-'));
const INPUT_FILE = join(srcDir, 'input.mp4');
execFileSync('ffmpeg', [
...
process.on('exit', () => { import('node:fs').then(fs => fs.rmSync(srcDir, { recursive: true, force: true })).catch(() => {}); });
